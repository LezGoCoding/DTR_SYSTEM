...
class Services{
	...
	protected static $tbl_name = "services";
	function db_fields(){
		global $mydb;
	...
	function single_service($id=0){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where SERVICE_ID = :id LIMIT 1", [':id' => $id]);
	...
	public function create() {
		...
		try {
	        ...
	        $attributes['DATE_CREATED'] = date('Y-m-d H:i:s'); // Or use Carbon::now()->toDateTimeString() if available
